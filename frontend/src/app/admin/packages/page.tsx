...
export default function AdminPackagesPage() {
  ...
  const fetchPackages = async () => {
    try {
      const res = await fetch('http://localhost:3001/api/subscriptions/packages');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = form.id 
        ? `http://localhost:3001/api/subscriptions/packages/${form.id}`
        : 'http://localhost:3001/api/subscriptions/packages';
